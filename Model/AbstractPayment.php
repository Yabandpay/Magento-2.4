...
abstract class AbstractPayment extends AbstractMethod
{
    ...
    public function startTransaction(Order $order)
    {
        try{
            ...
            $this->yaBandWechatPayHelper->addTolog('error', 'startTransaction:' .$e->getMessage());
            return $this->yaBandWechatPayHelper->getCheckoutUrl();
        }
    }
    ...
    public function processTransaction(array $orderInfo)
    {
        try{
            ...
            if($status == YaBandWechatPayHelper::PAY_PAID && $order->getStatus() !== Payment::PAY_PROCESSING){
                $plugin_status = $this->yaBandWechatPayHelper->getStatusPending();
                if($plugin_status==$order->getStatus()){
            ...
            $this->yaBandWechatPayHelper->addTolog('error', 'processTransaction:' .$msg);
            return $msg;
        }
    }
